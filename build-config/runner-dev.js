...
function startElectron() {
  const args = ['--inspect=5858', path.join(__dirname, '../dist/main.js')];
...
function printDevServerError(name, err) {
  ...
  console.error(
    chalk.red(
      `${name} dev server cannot listen on ${err.address ?? '127.0.0.1'}:${err.port ?? 'unknown'} (${err.code})`,
    ),
  );
  console.error(
    chalk.yellow(`Try: CORAL_DEV_HOST=localhost ${portEnv}=${fallbackPort} npm run dev`),
  );
}
...
async function shutdown(exitCode = 0) {
  ...
  killElectron();
  await Promise.all(
    closeHandlers.splice(0).map(async (close) => {
      await close();
    }),
  );
