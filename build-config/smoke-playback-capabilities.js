...
record('external decoder probe is typed and non-executing', () => {
  ...
  assertIncludes(read('src/main/modules/winMain/externalDecoderRuntime.ts'), [
...
record('local playback resolver routes native and external formats separately', () => {
  ...
  assertIncludes(read('src/renderer-react/services/playerRuntime/htmlAudioRuntime.ts'), [
  ...
  assertIncludes(read('src/renderer-react/services/playerRuntime/localAudioDecodeService.ts'), [
  ...
  assertIncludes(read('src/renderer-react/stores/domains/playerStore.ts'), [
    'needsExternalDecoder',
    'FFmpeg|外部解码|解码器|DSD|SACD|WAV|PCM|foobar',
  ], 'src/renderer-react/stores/domains/playerStore.ts')
  assertIncludes(read('src/renderer-react/components/player/PlayBar.tsx'), [
    'player.needsExternalDecoder',
    'configureExternalDecoder',
    '配置解码器',
  ], 'src/renderer-react/components/player/PlayBar.tsx')
  assertIncludes(read('src/renderer-react/components/player/PlayDetailOverlay.tsx'), [
    'errorActionNode',
    'player.needsExternalDecoder',
    'configureExternalDecoder',
    '配置解码器',
...
record('user api playback selection validates musicUrl capability', () => {
  ...
  assertIncludes(read('src/renderer-react/features/settings/SettingsRoutePanel.tsx'), [
...
record('feature entrypoints remain visible for local files and source plugins', () => {
  assertIncludes(read('src/renderer-react/app/AppShell.tsx'), [
    '本地文件',
    '添加音源',
    'importLocalAudio',
    'importUserApiFile',
  ...
  assertIncludes(read('src/renderer-react/stores/domains/uiStore.ts'), [
    'UiQuickAction',
    'configureExternalDecoder',
    'requestQuickAction',
    'consumeQuickAction',
