...
record('package identity is coral', () => {
  ...
  assert(pkg.macLanguagesInfoPlistStrings.en.CFBundleDisplayName === 'Coral Music', 'mac en display name mismatch')
  assert(pkg.macLanguagesInfoPlistStrings.zh_CN.CFBundleDisplayName === '珊瑚音乐', 'mac zh_CN display name mismatch')
})

record('package repository metadata is not upstream', () => {
  const pkg = JSON.parse(read('package.json'))
  assert(pkg.repository.type === 'git', 'repository.type should be git')
  assert(pkg.repository.url === '', 'repository.url should stay empty until the Coral repository is known')
