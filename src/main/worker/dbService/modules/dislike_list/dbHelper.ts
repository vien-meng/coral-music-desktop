...
export const queryDislikeList = () => {
  ...
  return queryStatement.all() as Coral.DBService.DislikeInfo[];
};
 ...
 * @param infos 列表
 */
export const insertDislikeList = async (infos: Coral.DBService.DislikeInfo[]) => {
  const db = getDB();
  const insertStatement = createInsertStatement();
  db.transaction((infos: Coral.DBService.DislikeInfo[]) => {
 ...
 * @param infos 列表
 */
export const overwirteDislikeList = async (infos: Coral.DBService.DislikeInfo[]) => {
  const db = getDB();
...
//  * @param urlInfo 列表
//  */
// export const updateDislikeList = async(infos: Coral.DBService.DislikeInfo[]) => {
//   const db = getDB()
