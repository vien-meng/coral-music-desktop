...
export default () => {
  mainHandle<LX.List.UserListInfo[]>(PLAYER_EVENT_NAME.list_get, async () =>
    global.lx.worker.dbService.getAllUserList(),
  );
  ...
  mainHandle<string, LX.Music.MusicInfo[]>(
    ...
    async ({ params: listId }) => global.lx.worker.dbService.getListMusics(listId),
  );
  ...
  mainHandle<LX.List.ListActionCheckMusicExistList, boolean>(
    ...
    async ({ params: { listId, musicInfoId } }) =>
      global.lx.worker.dbService.checkListExistMusic(listId, musicInfoId),
  );
  mainHandle<string, string[]>(
    PLAYER_EVENT_NAME.list_music_get_list_ids,
    async ({ params: musicInfoId }) => global.lx.worker.dbService.getMusicExistListIds(musicInfoId),
  );
};
