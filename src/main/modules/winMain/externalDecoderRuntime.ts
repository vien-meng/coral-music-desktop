  ...
  await new Promise<void>((resolve, reject) => {
    ...
    child.on('error', (error: NodeJS.ErrnoException) => {
      ...
      if (error.code === 'ENOENT') {
        reject(new Error('未找到 FFmpeg，请先安装 FFmpeg，或在“设置 > 本地解码”填写 ffmpeg 可执行文件路径。'))
        return
      }
...
export const transcodeExternalDecoder = async(
  ...
  if (params.provider === 'foobar2000') {
    ...
    throw new Error('Foobar2000 已支持路径探测，但当前还没有可靠的无交互转码适配；请先使用 FFmpeg 作为外部解码器播放 DSD/SACD 文件。')
  }
