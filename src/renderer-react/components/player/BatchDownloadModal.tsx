...
export const BatchDownloadModal = ({ musics, onClose }: BatchDownloadModalProps) => {
  ...
  const onlineMusics = musics.filter((musicInfo): musicInfo is LX.Music.MusicInfoOnline => musicInfo.source !== 'local')
  const availableQualities = batchDownloadQualities.filter((quality) => {
    ...
    return onlineMusics.every((musicInfo) => {
      return QUALITYS
        .slice(qualityIndex)
        .some(fallbackQuality => Boolean(musicInfo.meta._qualitys[fallbackQuality]))
    })
  })
