    ...
    <div className="coral-comment-item">
      <Flex align="flex-start" justify="space-between" gap={12}>
        <div className="coral-comment-user">
          <Text strong ellipsis>{comment.userName || '匿名用户'}</Text>
          <Space size={8} wrap>
...
export const MusicCommentPanel = observer(() => {
  ...
  const loadComments = useCallback(async(kind: MusicCommentKind, page = 1): Promise<void> => {
    if (!musicInfo) return
    ...
    if (state.error) {
      return (
        <Empty
          ...
          description="评论读取失败"
        >
          <Button onClick={() => { void loadComments(activeKind, state.page) }}>重试</Button>
        </Empty>
      )
    }
    ...
    <div className="coral-playdetail-comments">
      <Flex align="center" justify="space-between" className="coral-playdetail-comments-header">
        <div>
          ...
          <Title level={5} ellipsis>{musicInfo?.name ?? '等待播放'}</Title>
        </div>
        <Space size={8}>
          <Button
            aria-label="刷新评论"
            icon={<ReloadOutlined />}
            shape="circle"
            onClick={() => { void loadComments(activeKind, state.page) }}
          />
          <Button
            aria-label="关闭评论"
            icon={<CloseOutlined />}
            shape="circle"
            onClick={() => { player.closeCommentPanel() }}
          />
        </Space>
      </Flex>
      <Tabs
      ...
      />
      <div className="coral-playdetail-comments-body">
        {renderList()}
      </div>
    </div>
  )
