...
export const SoundEffectBtn = observer(() => {
  ...
  const hasCustomEq = useMemo(() => {
    return eqFrequencies.some(frequency => {
      return appSetting?.[`player.soundEffect.biquadFilter.hz${frequency}`] !== 0
    })
  }, [appSetting])
    ...
    <>
      <Tooltip title="音效">
        <Button
          ...
          shape="circle"
          type={hasCustomEq || appSetting?.['player.soundEffect.panner.enable'] ? 'primary' : 'default'}
        ...
        <Space direction="vertical" size="middle" className="coral-wide">
              ...
              {eqFrequencies.map(frequency => (
                <div key={frequency} className="coral-eq-slider">
                  <Slider
                    ...
                    onChange={value => {
                      updateSetting({ [`player.soundEffect.biquadFilter.hz${frequency}`]: Math.round(value) })
                    }}
                  />
          ...
          <section>
            ...
            <Slider
              ...
              max={200}
              step={1}
              value={Math.round((appSetting?.['player.soundEffect.pitchShifter.playbackRate'] ?? 1) * 100)}
              marks={{ 50: '0.5x', 100: '1x', 200: '2x' }}
              onChange={value => {
                updateSetting({ 'player.soundEffect.pitchShifter.playbackRate': Number((value / 100).toFixed(2)) })
              }}
            />
          </section>
        </Space>
