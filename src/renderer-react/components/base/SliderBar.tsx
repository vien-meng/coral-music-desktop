...
export const SliderBar = ({
  ...
  const getSliderWidth = useCallback((): number => containerRef.current?.clientWidth ?? 0, []);

  const emitSteppedValue = useCallback(
  ...
  const handleMouseDown = useCallback(
    (event: React.MouseEvent) => {
      ...
      dragStateRef.current.msDownRatio =
        range === 0 ? 0 : (getSteppedValue(rawValue) - min) / range;
      setIsDragging(true);
    },
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (event: MouseEvent): void => {
      ...
      if (!width) return;
      const r =
        dragStateRef.current.msDownRatio + (event.clientX - dragStateRef.current.msDownX) / width;
