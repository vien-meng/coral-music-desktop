...
export interface VirtualizedListHandle {
  scrollTo: (scrollTop: number, animate?: boolean, onScrollEnd?: (result: boolean | 'canceled') => void) => void
  scrollToIndex: (index: number, offset?: number, animate?: boolean, onScrollEnd?: (result: boolean | 'canceled') => void) => void
  getScrollTop: () => number
}
...
const VirtualizedListInner = <T,>(props: VirtualizedListProps<T>, ref: React.Ref<VirtualizedListHandle>) => {
  ...
  const createList = useCallback((start: number, end: number): Array<ViewItem<T>> => {
  ...
  const updateView = useCallback((currentScrollTop?: number) => {
    ...
    const continuous = currentStartIndex <= endIndexRef.current && currentEndIndex >= startIndexRef.current
  ...
  const handleReset = useCallback((newList: T[]) => {
  ...
  const scrollTo = useCallback((targetScrollTop: number, animate = false, onScrollEnd?: (result: boolean | 'canceled') => void) => {
    const container = containerRef.current
    if (!container) return
  ...
  const scrollToIndex = useCallback((index: number, offset = 0, animate = false, onScrollEnd?: (result: boolean | 'canceled') => void) => {
  ...
  useImperativeHandle(ref, (): VirtualizedListHandle => ({
    scrollTo,
    scrollToIndex,
    getScrollTop,
