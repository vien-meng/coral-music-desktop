...
export const LeaderboardRoutePanel = observer(() => {
  ...
  const maxPage = leaderboard.listDetailInfo.total > 0
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ padding: '8px 15px', flex: 'none' }}>
        <Space wrap style={{ width: '100%' }}>
          ...
          <Button
            ...
            onClick={() => { leaderboard.loadBoards().catch(() => {}) }}
          >
            榜单
          </Button>
          <OnlinePager
          ...
          <Segmented
            ...
            onChange={value => { setViewMode(value as 'board' | 'music') }}
          />
        </Space>
      </div>
      ...
      {leaderboard.detailError
        ? <Alert showIcon type="error" message={leaderboard.detailError} closable style={{ margin: '0 15px 8px', flex: 'none' }} />
        : null}
      {list.actionError
        ? <Alert showIcon type="error" message={list.actionError} closable style={{ margin: '0 15px 8px', flex: 'none' }} />
        : null}

      {/* Board list + selected board detail */}
      <div className="scroll" style={{ flex: 1, overflowY: 'auto', padding: '0 15px 15px', display: 'flex', gap: 15 }}>
        {/* Board list sidebar */}
