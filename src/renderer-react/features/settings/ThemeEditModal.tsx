...
const COLOR_FIELDS = [
  { key: '--color-primary', label: '主题主色', swatches: ['#f0645a', '#3c9eff', '#07c160', '#ff9800', '#9c27b0', '#e91e63', '#00bcd4', '#4caf50', '#ff5722', '#673ab7', '#795548', '#607d8b', '#ffeb3b', '#cddc39', '#009688'] },
  { key: '--color-1000', label: '字体颜色', swatches: ['rgb(33, 33, 33)', 'rgb(255, 255, 255)'] },
  { key: '--color-app-background', label: '应用背景色', swatches: ['rgba(255, 255, 255, 0)', 'rgba(255, 255, 255, 0.5)', 'rgba(255, 255, 255, 1)'] },
  { key: '--color-nav-font', label: '侧栏字体色', swatches: null },
  { key: '--color-main-background', label: '主区域背景色', swatches: ['rgba(255, 255, 255, 0.7)', 'rgba(255, 255, 255, 0.5)', 'rgba(0, 0, 0, 0.3)', 'rgba(0, 0, 0, 0.5)', 'rgba(0, 0, 0, 0.7)'] },
  { key: '--color-badge-primary', label: '徽章主色（无损）', swatches: null },
  { key: '--color-badge-secondary', label: '徽章次色（高品）', swatches: ['#ff5722', '#ff9800', '#ffc107', '#ffeb3b', '#cddc39', '#8bc34a', '#4caf50', '#009688', '#00bcd4', '#03a9f4', '#2196f3', '#3f51b5', '#9c27b0'] },
  { key: '--color-badge-tertiary', label: '徽章三级色（kw）', swatches: ['#ff5722', '#ff9800', '#ffc107', '#ffeb3b', '#cddc39', '#8bc34a', '#4caf50', '#009688', '#00bcd4', '#03a9f4', '#2196f3', '#3f51b5', '#9c27b0'] },
  { key: '--color-btn-hide', label: '隐藏按钮色', swatches: ['rgba(0, 0, 0, 0.5)', 'rgba(255, 255, 255, 0.5)', 'rgba(0, 0, 0, 0.3)', 'rgba(255, 255, 255, 0.3)'] },
  { key: '--color-btn-min', label: '最小化按钮色', swatches: ['rgba(0, 0, 0, 0.5)', 'rgba(255, 255, 255, 0.5)', 'rgba(0, 0, 0, 0.3)', 'rgba(255, 255, 255, 0.3)'] },
  { key: '--color-btn-close', label: '关闭按钮色', swatches: ['#ff5f56', '#ffbd2e', '#27c93f', '#ff5f5f'] },
] as const
...
export const ThemeEditModal = observer(({ onClose, onSaved, open, themeId }: ThemeEditModalProps) => {
      ...
      pickr.on('change', (color: Pickr.HSVaColor) => {
        ...
        setColors(prev => {
          ...
          if (field.key === '--color-primary') {
            ...
            const themeColors = createThemeColors(rgba, fontColor, isDarkRef.current, isDarkFontRef.current)
  ...
  const handleSelectBgImg = async(): Promise<void> => {
    ...
    try {
      const result = await appService.showSelectDialog({
        ...
        properties: ['openFile'],
        filters: [
          { name: 'Images', extensions: ['jpg', 'jpeg', 'png', 'gif', 'svg', 'webp', 'bmp', 'avif'] },
        ],
      })
  ...
  const buildTheme = (): LX.Theme => {
    ...
    if (themeColors['--color-primary']) {
      ...
      const generated = createThemeColors(themeColors['--color-primary'], fontColor, isDark, isDarkFont)
    ...
    return {
      ...
      config: {
        ...
        extInfo: {
          ...
          '--color-main-background': extInfo['--color-main-background'] ?? 'rgba(255, 255, 255, 0.7)',
  ...
  const handleSave = async(): Promise<void> => {
    ...
    try {
      ...
      if (oldTheme?.config.extInfo?.['--background-image'] &&
          oldTheme.config.extInfo['--background-image'] !== 'none' &&
          oldTheme.config.extInfo['--background-image'] !== basename(bgPath)) {
        try {
          await removeFile(joinPath(themeInfo.dataPath, oldTheme.config.extInfo['--background-image']))
        } catch {
          // ignore
        }
      }
  ...
  return (
    ...
    >
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16, padding: '8px 0', maxHeight: '60vh', overflow: 'auto' }}>
        <div>
          <Text type="secondary" style={{ display: 'block', marginBottom: 8 }}>基础颜色</Text>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
            {COLOR_FIELDS.slice(0, 5).map(field => (
              <div key={field.key} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4 }}>
                <div ref={el => { containerRefs.current[field.key] = el }} />
        ...
        <div>
          <Text type="secondary" style={{ display: 'block', marginBottom: 8 }}>背景图片</Text>
          <Space>
            <Button size="small" onClick={handleSelectBgImg}>选择图片</Button>
            {bgPath ? <Button size="small" danger onClick={handleRemoveBgImg}>移除</Button> : null}
            {bgPath ? <Text type="secondary" style={{ fontSize: 12 }}>{isUrl(bgPath) ? bgPath : basename(bgPath)}</Text> : null}
          </Space>
        </div>

        <div>
          <Text type="secondary" style={{ display: 'block', marginBottom: 8 }}>徽章颜色</Text>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div>
          <Text type="secondary" style={{ display: 'block', marginBottom: 8 }}>控制按钮颜色</Text>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 12 }}>
        ...
        <div>
          <Text type="secondary" style={{ display: 'block', marginBottom: 8 }}>主题信息</Text>
          <Space direction="vertical" style={{ width: '100%' }}>
            ...
            />
            <Space wrap>
              <Checkbox checked={isDark} onChange={event => { setIsDark(event.target.checked) }}>
                深色主题
              </Checkbox>
              <Checkbox checked={isDarkFont} onChange={event => { setIsDarkFont(event.target.checked) }}>
                深色字体
              </Checkbox>
              <Checkbox checked={preview} onChange={event => { setPreview(event.target.checked) }}>
                实时预览
              </Checkbox>
            </Space>
          </Space>
