...
const compareText = (left = '', right = ''): number => {
  return left.localeCompare(right, 'zh-Hans-CN')
}

const normalizeDuplicateMusicName = (name: string): string => {
  return name.toLocaleLowerCase().replace(duplicateVariantPattern, '').replace(duplicatePunctuationPattern, '') ||
...
const sortMusicInfos = (
  ...
  return musicInfos.slice().sort((left, right) => {
    let result = 0
...
const getDuplicateMusicReviewItems = (musicInfos: LX.Music.MusicInfo[]): DuplicateMusicReviewItem[] => {
...
export const LocalListRoutePanel = observer(() => {
  ...
  const isAllSelected = Boolean(filteredMusicIds.length) && filteredMusicIds.every(id => selectedMusicIdSet.has(id))
  ...
  const handleSaveCurrentSort = (): void => {
    ...
    void list.replaceSelectedMusicOrder(sortMusicInfos(list.selectedMusics, sortField, sortDirection))
  }
  ...
  const handleCopyMusicName = (musicInfo: LX.Music.MusicInfo): void => {
    ...
    void navigator.clipboard.writeText(musicName).then(() => {
  ...
  const handleConfirmSourceToggle = (targetMusicInfo: LX.Music.MusicInfo): void => {
    ...
    const duplicateTarget = list.selectedMusics.some(musicInfo => musicInfo.id === targetMusicInfo.id)
    const replaceMusic = async(removeDuplicateTarget: boolean): Promise<void> => {
      await list.replaceSelectedListMusic(sourceToggleMusic.id, targetMusicInfo, removeDuplicateTarget)
    ...
    <Space direction="vertical" size="middle" className="coral-wide coral-local-page">
      <Space wrap className="coral-route-controls coral-local-primary-controls">
        ...
        {normalizedFilterText
          ? <Tag color="blue">筛选 {filteredMusics.length}</Tag>
          : null}
      </Space>
      ...
      {list.hydrateError
        ? <Alert showIcon type="error" message={list.hydrateError} />
        : null}
      {list.actionError
        ? <Alert showIcon type="error" message={list.actionError} />
        : null}
      {list.musicError
        ? <Alert showIcon type="error" message={list.musicError} />
        : null}
              ...
              <PlainListMeta
                ...
                description={(
                  <Space wrap>
                    ...
                    {item.musicInfo.meta.albumName ? <span>{item.musicInfo.meta.albumName}</span> : null}
                  </Space>
                )}
              />
            </PlainListItem>
