...
export class ListStore {
  ...
  async moveMusicsToList(
    ...
    if (!sourceListId || !targetListId || sourceListId === targetListId || !musicInfos.length) return

    this.isAddingMusic = true
    this.actionError = null

    try {
      await listService.moveListMusics(sourceListId, targetListId, musicInfos, addMusicLocationType)
      const movedIdSet = new Set(musicInfos.map(musicInfo => musicInfo.id))
      this.selectedMusics = this.selectedMusics.filter(musicInfo => !movedIdSet.has(musicInfo.id))
    } catch (error) {
      this.actionError = error instanceof Error ? error.message : String(error)
    } finally {
      this.isAddingMusic = false
  ...
  async moveSelectedMusicsToPosition(
    ...
    const remainingMusics = this.selectedMusics.filter(musicInfo => !movedIdSet.has(musicInfo.id))
  ...
  async replaceSelectedListMusic(
    ...
    const duplicateIndex = this.selectedMusics.findIndex(musicInfo => musicInfo.id === newMusicInfo.id)
    ...
    const remainingMusics = this.selectedMusics.filter(musicInfo => !removeIdSet.has(musicInfo.id))
    const targetPosition = duplicateIndex > -1 && duplicateIndex < oldIndex ? oldIndex - 1 : oldIndex
  ...
  async renameSelectedList(name: string): Promise<void> {
    ...
    try {
      ...
      this.userLists = this.userLists.map(list => list.id === updatedList.id ? updatedList : list)
    } catch (error) {
      this.actionError = error instanceof Error ? error.message : String(error)
    } finally {
      this.isMutatingList = false
  ...
  async removeMusicsFromSelectedList(musicIds: string[]): Promise<void> {
    ...
    try {
      ...
      this.selectedMusics = this.selectedMusics.filter(musicInfo => !removeIdSet.has(musicInfo.id))
    } catch (error) {
      this.actionError = error instanceof Error ? error.message : String(error)
    } finally {
      this.isMutatingMusic = false
