...
export class DislikeStore {
  ...
  hasDislike(info: LX.Music.MusicInfo | LX.Download.ListItem): boolean {
    ...
    return this.dislikeInfo.musicNames.has(name) ||
  ...
  private syncRuleCount(): void {
    this.ruleCount = this.dislikeInfo.musicNames.size + this.dislikeInfo.singerNames.size + this.dislikeInfo.names.size
  }
}
