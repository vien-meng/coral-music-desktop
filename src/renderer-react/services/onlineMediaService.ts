...
const normalizeLyricInfo = (lyricInfo?: Partial<LX.Music.LyricInfo> | null): LX.Music.LyricInfo => {
  return {
...
export const getOnlineLyricInfo = async(
  ...
  const lyricRequest = getSourceSdk(sdk, musicInfo.source)?.getLyric?.(toOldMusicInfo(musicInfo), true)
...
export const getOnlinePicUrl = async(
  musicInfo: LX.Music.MusicInfoOnline,
): Promise<string> => {
