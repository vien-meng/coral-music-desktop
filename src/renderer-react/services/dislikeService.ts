...
export const onAddDislikeMusicInfos = (listener: (infos: LX.Dislike.DislikeMusicInfo[]) => void): (() => void) => {
  if (!isElectronRenderer()) return () => {}
  return ipcClient.on(ipcChannels.dislike.addDislikeMusicInfos, listener)
}

export const onOverwriteDislikeMusicInfos = (listener: (rules: LX.Dislike.DislikeRules) => void): (() => void) => {
  if (!isElectronRenderer()) return () => {}
