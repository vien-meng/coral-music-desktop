...
export const getMusicInfoRaw = async (hash) =>
  Promise.all(createGetMusicInfosTask([{ hash }])).then((data) => data.flat()[0]);

export const getMusicInfo = async (hash) => getMusicInfos([{ hash }]).then((data) => data[0]);

export const getMusicInfosByList = (list) =>
  getMusicInfos(list.map((item) => ({ hash: item.hash })));
