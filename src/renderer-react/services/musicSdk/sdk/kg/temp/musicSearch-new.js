...
export default {
  ...
  filterList(raw) {
    ...
    raw.forEach((item) => {
      ...
      if (item.HQ != undefined) {
        const size = sizeFormate(item.HQ.FileSize);
      ...
      if (item.SQ != undefined) {
        const size = sizeFormate(item.SQ.FileSize);
      ...
      if (item.Res != undefined) {
        const size = sizeFormate(item.Res.FileSize);
  ...
  search(str, page = 1, limit, retryNum = 0) {
    ...
    return this.musicSearch(str, page, limit).then((data) => {
      const list = this.handleResult(data.lists);
