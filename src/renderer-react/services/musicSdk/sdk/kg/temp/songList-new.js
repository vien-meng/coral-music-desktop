...
export default {
  ...
  async getUserListDetailBySpecialId(id, page, tryNum = 0) {
    ...
    const list = await getMusicInfosByList(JSON.parse(listData[1]));
    let name;
    let pic;
    if (listInfo) {
      name = listInfo[1];
  ...
  filterListByCollectionId(rawData) {
    ...
    rawData.forEach((item) => {
      ...
      item.relate_goods.forEach((data) => {
        const size = sizeFormate(data.size);
        switch (data.level) {
          case 2:
  ...
  async getUserListDetailByCode(id, page = 1) {
    ...
    if (codeInfo.userid != null) {
      ...
      const list = await getMusicInfosByList(songList || codeInfo.list);
      return {
        list,
        page: 1,
  ...
  async getUserListDetail3(chain, page) {
    ...
    const list = await getMusicInfosByList(songInfo.list);
  ...
  createGetListDetail2Task(id, total) {
    ...
    while (total) {
      ...
      total -= limit;
      page += 1;
      const params = `appid=1058&global_specialid=${id}&specialid=0&plat=0&version=8000&page=${
        page
      }&pagesize=${
        limit
  ...
  async getUserListDetail2(global_collection_id) {
    ...
    const params = `appid=1058&specialid=0&global_specialid=${
      id
    ...
    const list = await getMusicInfosByList(songInfo);
  ...
  async getUserListDetail(link, page, retryNum = 0) {
    ...
    if (typeof body === 'string') {
      ...
      if (body.includes('list_id = "') && link.includes('album/info'))
        return album.getAlbumDetail(
          body.replace(/^[\s\S]+?list_id = "(\w+)"[\s\S]+?$/, '$1'),
          page,
        );
    }
    if (location) {
