...
export const basename = (filePath: string, ext?: string): string => getPath().basename(filePath, ext)
...
export const readFile = async(filePath: string): Promise<Buffer> => {
  return await getFsPromises().readFile(filePath)
}
...
export const removeFile = async(filePath: string): Promise<void> => {
  await getFsPromises().unlink(filePath).catch((error: NodeJS.ErrnoException) => {
