...
const createFontDarkColors = (rgbaColor: string, isDarkFont: boolean): Record<string, string> => {
...
const createFontColors = (
  ...
  for (let i = 1; i < 21; i += 1) {
    colors[`--color-${String(1000 - 50 * i).padStart(3, '0')}`] = rgbLinearShade(
      step * i,
      rgbaColor,
    );
  }
...
export const createThemeColors = (
  ...
  for (let i = 1; i < 10; i += 1) {
    ...
    for (let j = 1; j < 10; j += 1) {
      colors[`--color-primary-light-${i * 100}-alpha-${j * 100}`] = rgbAlphaShade(
        0.1 * j,
        preColor,
      );
    }
