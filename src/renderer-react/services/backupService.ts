...
export const exportPlayListToText = async(savePath: string, isMerge: boolean): Promise<void> => {
  ...
  try {
    ...
    if (isMerge) {
      const content = lists.map(l => l.list.map(m => `${m.name}  ${m.singer}  ${m.meta.albumName ?? ''}`).join('\n')).join('\n\n')
...
export const exportPlayListToCsv = async(savePath: string, isMerge: boolean, header: string): Promise<void> => {
  if (!isElectronRenderer()) return
  try {
    const lists = await getAllLists()
    ...
    if (isMerge) {
      const content = header + lists.map(l => l.list.map(m => `${filterStr(m.name)},${filterStr(m.singer)},${filterStr(m.meta.albumName ?? '')}`).join('\n')).join('\n')
      await saveStrToFile(savePath, content)
    } else {
      for (const list of lists) {
