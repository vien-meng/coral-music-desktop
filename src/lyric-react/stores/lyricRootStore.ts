...
import {
  ...
  applyLyricLanguage,
  applyLyricTheme,
} from '../services/lyricEnvironment';
...
export class LyricRootStore {
  ...
  private mergeConfig(config: Partial<Coral.DesktopLyric.Config>): void {
    ...
    if (shouldUpdateVertical)
      this.timeline.setVertical(this.config['desktopLyric.direction'] === 'vertical');
    ...
    if (shouldSyncAudioVisualization && !this.config['desktopLyric.audioVisualization'])
      this.analyserData = null;
