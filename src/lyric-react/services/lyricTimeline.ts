...
const getDisplayLyric = (lyrics: {
  lyric: string
  lxlyric: string | null
}, config: LX.DesktopLyric.Config): string => {
  return config['player.isPlayLxlrc'] && lyrics.lxlyric ? lyrics.lxlyric : lyrics.lyric
}

const getExtendedLyrics = (lyrics: {
  rlyric: string | null
  tlyric: string | null
}, config: LX.DesktopLyric.Config): string[] => {
...
export class LyricTimelineService {
  ...
  setLyric(lyrics: {
    ...
    rlyric: string | null
    tlyric: string | null
  }, config: LX.DesktopLyric.Config): void {
    this.player?.setLyric(
      getDisplayLyric(lyrics, config),
      getExtendedLyrics(lyrics, config),
    )
  }
