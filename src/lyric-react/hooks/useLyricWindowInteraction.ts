...
const isWindows = (): boolean => {
  return new URLSearchParams(globalThis.location?.search).get('os') === 'windows'
}

const getTouchPoint = (event: TouchEvent | ReactTouchEvent): Touch | null => {
  return event.changedTouches.length ? event.changedTouches[0] : null
}

const getResizeBounds = (state: ResizeState, clientX: number, clientY: number): LX.DesktopLyric.NewBounds => {
