...
41. Step 150 tightened local metadata and WebM playback. Local metadata enrichment now uses `music-metadata` first, MP3/FLAC/WAV header fallback second, and WebM `web-demuxer` media info for audio-stream bitrate/sample-rate recovery. WebM playback first tries renderer-side `web-demuxer` demux plus WebCodecs `AudioDecoder` into the existing decoded AudioBuffer runtime, then falls back to the main-process `audio-decode` path.

Recommended next implementation batch:

1. Step 151: revisit UI/Electron click-through automation once the packaging/runtime environment is less noisy.
2. Step 152: run a focused visual pass on Settings/List/PlayBar when a stable app window is available.
3. Step 153: when the real Coral Music repository is known, set `repository.url`, `bugs.url`, `homepage`, README badges, and publish owner/repo workflow docs together.
