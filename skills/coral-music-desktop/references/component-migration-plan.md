...
37. Step 146 fixed local FLAC playback without requiring online sources or external FFmpeg configuration. The player now probes `Audio.canPlayType()` before treating a local extension as browser-native, and local/downloaded FLAC falls back to the bundled `@wasm-audio-decoders/flac` decoder, which writes an in-memory WAV Blob URL for the existing audio runtime.
