...
- Added the new capability requirements for local audio playback, Foobar2000-style decoder integration, and LX Music source plugin configuration to the formal migration plan.
...
- Preserved the existing LX Music User API runtime as the source-plugin configuration path instead of introducing a competing plugin system.
...
- Added conservative AppSetting/default-setting keys for:
  ...
  - LX Music User API source-plugin preference flags.
...
4. Step 135: polish LX Music User API source-plugin validation and status display.
