...
同步功能完善，设置页同步/OpenAPI 配置增强，下载开关，字体大小与动画设置。

### 新增

- 同步自动启动：应用初始化时根据配置自动启动同步服务端或客户端，配置变更时自动重连
- 同步客户端链接码配对：首次连接时输入链接码，配对成功后不再保存
- 同步状态实时展示：服务端/客户端连接状态实时获取
- 同步冲突弹窗：`SyncConflictModal` 组件，同步冲突时提示用户
- OpenAPI 配置：设置页新增启用/端口/局域网绑定配置
- 下载功能开关：`download.enable` 设置项，禁用时阻止任务启动
- 字体大小 CSS 变量：`--coral-font-size` 响应设置中的字体大小
- 动画开关：`coral-reduce-motion` CSS 类响应动画设置
- README 增加移动端应用链接

### 优化

- 设置页同步开关改为带 loading 状态的异步操作
- 歌单收藏先判断是否已收藏，优化交互体验
- 在线列表音源显示可控：根据 `list.isShowSource` 设置控制是否显示来源
- 同步地址输入框 placeholder 改为 `https://sync.example.com`

### 修复

- 修复同步协议标识不兼容 LX 同步服务器的问题：`authMsg`/`msgConnect` 回退为 `lx-music` 前缀
